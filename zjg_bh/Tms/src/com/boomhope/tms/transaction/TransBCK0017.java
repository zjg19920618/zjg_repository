...
import com.boomhope.tms.message.out.Out03520ResBean;
import com.boomhope.tms.message.out.Out03520ResBodyBean;
...
public class TransBCK0017 extends BaseTransaction implements ServletContextListener{
	Logger logger = Logger.getLogger(TransBCK0017.class);
	ITransFlowService transFlowService;	// 交易流水服务
	IDeployMachineService deployMachineService = null;
	BCK0017ReqBean inReqBean;	// 渠道请求Bean
	BCK0017ResBean inResBean;	// 渠道响应Bean
	Out03520ReqBean	outReqBean;	// 前置请求Bean
	Out03520ResBean	outResBean;	// 前置响应Bean
	TransFlow transFlow;	// 交易流水Bean
	DeployMachine mac = null;
	public String handle(String msg) {
		try {
			transFlowService = (ITransFlowService) TransBCK0017.getBean("TransFlowService");
		...
		try {
			...
			logger.info("向前置发送03520请求"+outReqMsg);
			
			//发送前置交易请求 
			String outResMsg = new SocketClient().sendMsg(outReqMsg);
			logger.info("接收前置03520响应"+outResMsg);
	...
	private void addTransFlow() throws Exception {
		try {
			...
			transFlow.setTransCode("BCK_0017");
			transFlow.setTransName("回单机-存单打印状态变更");
	...
	private String makeResMsg() throws Exception{
		try {
			...
			BCK0017ResBodyBean bck0017ResBodyBean = new BCK0017ResBodyBean();
			
			bck0017ResBodyBean.setSVR_DATE(outResBean.getBODY().getSVR_DATE());//日期
			bck0017ResBodyBean.setSVR_JRNL_NO(outResBean.getBODY().getSVR_JRNL_NO());//流水号
			inResBean.setBody(bck0017ResBodyBean);
	...
	private Out03520ResBean makeOutResBean(String outResMsg) throws Exception{
		try {
			logger.info("前置返回报文：\n"+outResMsg);
			...
			reqXs.alias("ROOT", Out03520ResBean.class);
			reqXs.alias("HEAD", OutHeadBean.class);
			reqXs.alias("RESPONSE", OutResponseBean.class);
			reqXs.alias("BODY", Out03520ResBodyBean.class);
			
			return (Out03520ResBean)reqXs.fromXML(outResMsg);
		} catch (Exception e) {
	...
	private byte[] makeOutReqMsg() throws Exception{
		try {
			outReqBean = new Out03520ReqBean();
			Out03520ReqBodyBean out03520ReqBodyBean = new Out03520ReqBodyBean();
			out03520ReqBodyBean.setACCT_NO(inReqBean.getBody().getACCT_NO()); //卡号
			out03520ReqBodyBean.setSUB_ACCT_NO(inReqBean.getBody().getSUB_ACCT_NO()); //子账号
			out03520ReqBodyBean.setOPER_CHOOSE(inReqBean.getBody().getOPER_CHOOSE()); //操作选择
			
			outReqBean.setBODY(out03520ReqBodyBean);
			OutHeadBean outHeadBean = new OutHeadBean();
			outHeadBean.setTRAN_CODE("03520");
			...
			xs.alias("ROOT", Out03520ReqBean.class);
			xs.alias("HEAD", OutHeadBean.class);
			xs.alias("BODY", Out03520ReqBodyBean.class);
	...
	private BCK0017ReqBean makeInReqBean(String msg) throws Exception{
		/* 解析渠道请求报文*/
		XStream reqXs = new XStream(new DomDriver("UTF-8",new XStreamNameCoder()));
		
		reqXs.alias("Root", BCK0017ReqBean.class);
		reqXs.alias("Head", InReqHeadBean.class);
		reqXs.alias("Body", BCK0017ReqBodyBean.class);
		
		return (BCK0017ReqBean)reqXs.fromXML(msg);
	}
	@Override
