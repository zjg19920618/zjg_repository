...
import com.boomhope.tms.message.account.in.BCK03511ResBodyBean;
import com.boomhope.tms.message.account.out.Account03511ReqBean;
import com.boomhope.tms.message.account.out.Account03511ReqBodyBean;
import com.boomhope.tms.message.account.out.Account03511ResBean;
import com.boomhope.tms.message.account.out.Account03511ResBodyBean;
...
public class TransBCK03511 extends BaseTransaction implements ServletContextListener{
	Logger logger = Logger.getLogger(TransBCK03511.class);
	ITransFlowService transFlowService;	// 交易流水服务
	IDeployMachineService deployMachineService = null;
	BCK03511ReqBean inReqBean;	// 渠道请求Bean
	BCK03511ResBean inResBean;	// 渠道响应Bean
	Account03511ReqBean	outReqBean;	// 前置请求Bean
	Account03511ResBean	outResBean;	// 前置响应Bean
	DeployMachine mac = null;
	TransFlow transFlow;	// 交易流水Bean
	public String handle(String msg) {
		try {
			transFlowService = (ITransFlowService) TransBCK03511.getBean("TransFlowService");
		...
		try {
			...
			logger.info("向前置发送03511请求"+outReqMsg);
			
			//发送前置交易请求 
			String outResMsg = new SocketClient().sendMsg(outReqMsg);
			logger.info("接刷前置响应03511请求"+outReqMsg);
			...
			//然后将Account03511ResBean类中的数据放入BCK03511ResBean中，返回
			//返回信息
			return makeResMsg();
		} catch (Exception e) {
	...
	private void addTransFlow() throws Exception {
		try {
			...
			transFlow.setTransCode("BCK_03511");
			transFlow.setTransName("回单机-产品可开立额度信息查询");
	...
	private String makeResErrorMsg(String msg,String service) {
		...
		inResBean = new BCK03511ResBean();
		BCK03511ResBodyBean BCK03511ResBodyBean = new BCK03511ResBodyBean();
		
		inResBean.setBody(BCK03511ResBodyBean);
	...
	private String makeResMsg() throws Exception{
		try {
			inResBean = new BCK03511ResBean();
			BCK03511ResBodyBean BCK03511ResBodyBean = new BCK03511ResBodyBean();
			BCK03511ResBodyBean.setBALA(outResBean.getBODY().getBALA());
			BCK03511ResBodyBean.setSVR_DATE(outResBean.getBODY().getSVR_DATE());
			BCK03511ResBodyBean.setSVR_JRNL_NO(outResBean.getBODY().getSVR_JRNL_NO());
			inResBean.setBody(BCK03511ResBodyBean);
			InResHeadBean inResHeadBean = new InResHeadBean();
	...
	private Account03511ResBean makeOutResBean(String outResMsg) throws Exception{
		try {
			logger.info("前置返回报文：\n"+outResMsg);
			...
			reqXs.alias("ROOT", Account03511ResBean.class);
			reqXs.alias("HEAD", OutHeadBean.class);
			reqXs.alias("RESPONSE", OutResponseBean.class);
			reqXs.alias("BODY", Account03511ResBodyBean.class);
			
			return (Account03511ResBean)reqXs.fromXML(outResMsg);
		} catch (Exception e) {
	...
	private byte[] makeOutReqMsg() throws Exception{
		try {
			outReqBean = new Account03511ReqBean();
			Account03511ReqBodyBean accountBody = new Account03511ReqBodyBean();
			...
			xs.alias("ROOT", Account03511ReqBean.class);
			xs.alias("HEAD", OutHeadBean.class);
			xs.alias("BODY", Account03511ReqBodyBean.class);
	...
	private BCK03511ReqBean makeInReqBean(String msg) throws Exception{
		/* 解析渠道请求报文*/
		XStream reqXs = new XStream(new DomDriver("UTF-8",new XStreamNameCoder()));
		
		reqXs.alias("Root", BCK03511ReqBean.class);
		reqXs.alias("Head", InReqHeadBean.class);
		reqXs.alias("Body", BCK03511ReqBodyBean.class);
		
		return (BCK03511ReqBean)reqXs.fromXML(msg);
	}
