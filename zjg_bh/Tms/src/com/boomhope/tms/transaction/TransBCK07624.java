...
import com.boomhope.tms.message.in.bck.BCK07624ResBodyBean;
...
public class TransBCK07624 extends BaseTransaction implements ServletContextListener{
	Logger logger = Logger.getLogger(TransBCK07624.class);
	ITransFlowService transFlowService;	// 交易流水服务
	IFlowService flowService;   //业务流水服务
	IDeployMachineService deployMachineService = null;
	BCK07624ReqBean inReqBean;	// 渠道请求Bean
	BCK07624ResBean inResBean;	// 渠道响应Bean
	...
	public String handle(String msg) {
		try {
			transFlowService = (ITransFlowService) TransBCK07624.getBean("TransFlowService");
			deployMachineService = (IDeployMachineService) TransTms0001.getBean("deployMachineService");
			flowService = (IFlowService) TransBCK07624.getBean("flowService");
		} catch (Exception e) {
	...
	private void addTransFlow() throws Exception {
		try {
			...
			transFlow.setTransCode("BCK_07624");
	...
	private String makeResErrorMsg(String msg,String service) {
		...
		inResBean = new BCK07624ResBean();
		BCK07624ResBodyBean bck07624ResBodyBean = new BCK07624ResBodyBean();
		
		inResBean.setBody(bck07624ResBodyBean);
	...
	private String makeResMsg() throws Exception{
		try {
			inResBean = new BCK07624ResBean();
			BCK07624ResBodyBean bck07624ResBodyBean = new BCK07624ResBodyBean();
			bck07624ResBodyBean.setBALANCE(outResBean.getBODY().getBALANCE());
			bck07624ResBodyBean.setCR_DB_FLAG(outResBean.getBODY().getCR_DB_FLAG());
			bck07624ResBodyBean.setDEP_DAYS(outResBean.getBODY().getDEP_DAYS());
			bck07624ResBodyBean.setDUE_DATE(outResBean.getBODY().getDUE_DATE());
			bck07624ResBodyBean.setFILE_NAME(outResBean.getBODY().getFILE_NAME());
			bck07624ResBodyBean.setINTE_AMT(outResBean.getBODY().getINTE_AMT());
			bck07624ResBodyBean.setITEM_NO(outResBean.getBODY().getITEM_NO());
			bck07624ResBodyBean.setOPEN_INST(outResBean.getBODY().getOPEN_INST());
			bck07624ResBodyBean.setOPP_ACCT_NAME(outResBean.getBODY().getOPP_ACCT_NAME());
			bck07624ResBodyBean.setOPP_ACCT_NO(outResBean.getBODY().getOPP_ACCT_NO());
			bck07624ResBodyBean.setOPP_ITEM_NO(outResBean.getBODY().getOPP_ITEM_NO());
			bck07624ResBodyBean.setOPP_OPEN_INST(outResBean.getBODY().getOPP_OPEN_INST());
			bck07624ResBodyBean.setRATE(outResBean.getBODY().getRATE());
			bck07624ResBodyBean.setRATE_TAX(outResBean.getBODY().getRATE_TAX());
			bck07624ResBodyBean.setREAL_INTE(outResBean.getBODY().getREAL_INTE());
			bck07624ResBodyBean.setRATE_ITEM(outResBean.getBODY().getRATE_ITEM());
			bck07624ResBodyBean.setREAL_PRI(outResBean.getBODY().getREAL_PRI());
			bck07624ResBodyBean.setSTART_INT_DATE(outResBean.getBODY().getSTART_INT_DATE());
			bck07624ResBodyBean.setSUBSIDY_AMT(outResBean.getBODY().getSUBSIDY_AMT());
			bck07624ResBodyBean.setSVR_DATE(outResBean.getBODY().getSVR_DATE());
			bck07624ResBodyBean.setSVR_JRNL_NO(outResBean.getBODY().getSVR_JRNL_NO());
			bck07624ResBodyBean.setTAX_RATE(outResBean.getBODY().getTAX_RATE());
			bck07624ResBodyBean.setTRAN_AMT(outResBean.getBODY().getTRAN_AMT());
			bck07624ResBodyBean.setADVN_INIT(outResBean.getBODY().getADVN_INIT());
			bck07624ResBodyBean.setXFD_COUNT(outResBean.getBODY().getXFD_COUNT());
			bck07624ResBodyBean.setXFD_AMT(outResBean.getBODY().getXFD_AMT());
			inResBean.setBody(bck07624ResBodyBean);
			InResHeadBean inResHeadBean = new InResHeadBean();
...
//				logger.debug("*******存单账号："+acctNo);
	...
	private BCK07624ReqBean makeInReqBean(String msg) throws Exception{
		/* 解析渠道请求报文*/
		XStream reqXs = new XStream(new DomDriver("UTF-8",new XStreamNameCoder()));
		
		reqXs.alias("Root", BCK07624ReqBean.class);
		reqXs.alias("Head", InReqHeadBean.class);
		reqXs.alias("Body", BCK07624ReqBodyBean.class);
		
		return (BCK07624ReqBean)reqXs.fromXML(msg);
	}
