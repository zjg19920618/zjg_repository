...
import com.boomhope.tms.message.in.bck.BCK03522ResBodyBean;
 ...
 *
 */
public class TransBCK03522 extends BaseTransaction implements ServletContextListener{
	Logger logger = Logger.getLogger(TransBCK03522.class);
	...
	BCK03522ReqBean inReqBean;	// 渠道请求Bean
	BCK03522ResBean inResBean;	// 渠道响应Bean
	...
	public String handle(String msg) {
		try {
			transFlowService = (ITransFlowService) TransBCK03522.getBean("TransFlowService");
			deployMachineService = (IDeployMachineService) TransTms0001.getBean("deployMachineService");
			flowService = (IFlowService) TransBCK03522.getBean("flowService");
		} catch (Exception e) {
	...
	private void addTransFlow() throws Exception {
		try {
			...
			transFlow.setTransCode("BCK_03522");
	...
	private String makeResErrorMsg(String msg,String service) {
		...
		inResBean = new BCK03522ResBean();
		BCK03522ResBodyBean bck03522ResBodyBean = new BCK03522ResBodyBean();
		
		inResBean.setBody(bck03522ResBodyBean);
	...
	private String makeResMsg() throws Exception{
		try {
			inResBean = new BCK03522ResBean();
			BCK03522ResBodyBean bck03522ResBodyBean = new BCK03522ResBodyBean();
			bck03522ResBodyBean.setSVR_DATE(outResBean.getBODY().getSVR_DATE());
			bck03522ResBodyBean.setCARD_DATE(outResBean.getBODY().getCARD_DATE());
			bck03522ResBodyBean.setCARD_JRNL_NO(outResBean.getBODY().getCARD_JRNL_NO());
			bck03522ResBodyBean.setSVR_JRNL_NO(outResBean.getBODY().getSVR_JRNL_NO());
			bck03522ResBodyBean.setRATE(outResBean.getBODY().getRATE());
			bck03522ResBodyBean.setRED_INTEREST(outResBean.getBODY().getRED_INTEREST());
			bck03522ResBodyBean.setTAX_RATE(outResBean.getBODY().getTAX_RATE());
			bck03522ResBodyBean.setINTEREST_TAX(outResBean.getBODY().getINTEREST_TAX());
			bck03522ResBodyBean.setINVEST_INCOME(outResBean.getBODY().getINVEST_INCOME());
			bck03522ResBodyBean.setPAID_PRINCIPAL(outResBean.getBODY().getPAID_PRINCIPAL());
			bck03522ResBodyBean.setPAID_INTEREST(outResBean.getBODY().getPAID_INTEREST());
			bck03522ResBodyBean.setPAYOFF_INTEREST(outResBean.getBODY().getPAYOFF_INTEREST());
			bck03522ResBodyBean.setADVN_INIT(outResBean.getBODY().getADVN_INIT());
			bck03522ResBodyBean.setXFD_COUNT(outResBean.getBODY().getXFD_COUNT());//扣回消费豆数量
			bck03522ResBodyBean.setXFD_AMT(outResBean.getBODY().getXFD_AMT());//扣回消费豆金额
			inResBean.setBody(bck03522ResBodyBean);
			InResHeadBean inResHeadBean = new InResHeadBean();
...
//				logger.debug("*******存单账号："+acctNo);
	...
	private BCK03522ReqBean makeInReqBean(String msg) throws Exception{
		/* 解析渠道请求报文*/
		XStream reqXs = new XStream(new DomDriver("UTF-8",new XStreamNameCoder()));
		
		reqXs.alias("Root", BCK03522ReqBean.class);
		reqXs.alias("Head", InReqHeadBean.class);
		reqXs.alias("Body", BCK03522ReqBodyBean.class);
		
		return (BCK03522ReqBean)reqXs.fromXML(msg);
	}
