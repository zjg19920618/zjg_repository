...
import com.boomhope.tms.message.in.bck.BCK03517ResBodyBean;
 ...
 *
 */
public class TransBCK03517 extends BaseTransaction implements ServletContextListener{
	Logger logger = Logger.getLogger(TransBCK03517.class);
	...
	BCK03517ReqBean inReqBean;	// 渠道请求Bean
	BCK03517ResBean inResBean;	// 渠道响应Bean
	...
	public String handle(String msg) {
		try {
			transFlowService = (ITransFlowService) TransBCK03517.getBean("TransFlowService");
			deployMachineService = (IDeployMachineService) TransTms0001.getBean("deployMachineService");
			flowService = (IFlowService) TransBCK03517.getBean("flowService");
		} catch (Exception e) {
	...
	private void addTransFlow() throws Exception {
		try {
			...
			transFlow.setTransCode("BCK_03517");
	...
	private String makeResErrorMsg(String msg,String service) {
		...
		inResBean = new BCK03517ResBean();
		BCK03517ResBodyBean bck03517ResBodyBean = new BCK03517ResBodyBean();
		
		inResBean.setBody(bck03517ResBodyBean);
	...
	private String makeResMsg() throws Exception{
		try {
			inResBean = new BCK03517ResBean();
			BCK03517ResBodyBean bck03517ResBodyBean = new BCK03517ResBodyBean();
			bck03517ResBodyBean.setSVR_DATE(outResBean.getBODY().getSVR_DATE());
			bck03517ResBodyBean.setCARD_DATE(outResBean.getBODY().getCARD_DATE());
			bck03517ResBodyBean.setCARD_JRNL_NO(outResBean.getBODY().getCARD_JRNL_NO());
			bck03517ResBodyBean.setSVR_JRNL_NO(outResBean.getBODY().getSVR_JRNL_NO());
			bck03517ResBodyBean.setRATE(outResBean.getBODY().getRATE());
			bck03517ResBodyBean.setRED_INTEREST(outResBean.getBODY().getRED_INTEREST());
			bck03517ResBodyBean.setTAX_RATE(outResBean.getBODY().getTAX_RATE());
			bck03517ResBodyBean.setINTEREST_TAX(outResBean.getBODY().getINTEREST_TAX());
			bck03517ResBodyBean.setINVEST_INCOME(outResBean.getBODY().getINVEST_INCOME());
			bck03517ResBodyBean.setPAID_PRINCIPAL(outResBean.getBODY().getPAID_PRINCIPAL());
			bck03517ResBodyBean.setPAID_INTEREST(outResBean.getBODY().getPAID_INTEREST());
			bck03517ResBodyBean.setPAYOFF_INTEREST(outResBean.getBODY().getPAYOFF_INTEREST());
			bck03517ResBodyBean.setADVN_INIT(outResBean.getBODY().getADVN_INIT());
			bck03517ResBodyBean.setXFD_COUNT(outResBean.getBODY().getXFD_COUNT());//扣回消费豆数量
			bck03517ResBodyBean.setXFD_AMT(outResBean.getBODY().getXFD_AMT());//扣回消费豆金额
...
//				logger.debug("*******存单账号："+acctNo);
	...
	private BCK03517ReqBean makeInReqBean(String msg) throws Exception{
		/* 解析渠道请求报文*/
		XStream reqXs = new XStream(new DomDriver("UTF-8",new XStreamNameCoder()));
		
		reqXs.alias("Root", BCK03517ReqBean.class);
		reqXs.alias("Head", InReqHeadBean.class);
		reqXs.alias("Body", BCK03517ReqBodyBean.class);
		
		return (BCK03517ReqBean)reqXs.fromXML(msg);
	}
