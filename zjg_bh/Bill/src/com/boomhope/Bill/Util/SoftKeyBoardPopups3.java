...
public class SoftKeyBoardPopups3 extends JPopupMenu{
    ...
    public static void main(String[] args) {  
        ...
        final SoftKeyBoardPopups3 keyPopup = new SoftKeyBoardPopups3(password);  
    ...
    public SoftKeyBoardPopups3(JTextField textField) {  
        softKeyBoardPanel = new SoftKeyBoardPanel(textField, this);  
        softKeyBoardPanel.setBackground(Color.WHITE);
        softKeyBoardPanel.setPreferredSize(new Dimension(350, 260));  
        softKeyBoardPanel.setBorder(BorderFactory.createEmptyBorder());  
  
        add(softKeyBoardPanel);  
        softKeyBoardPanel.setLayout(new MigLayout("", "[]", "[]"));
...
//            return passwordField;
        ...
        public JTextField clickBackSpace() {  
            ...
            if (password != null && password.length > 0) { 
 			...
 			b=	a.substring(0, a.length()-2); 				 				
           	 System.out.println("转换金额"+b); 
            	 }   
            	 b1= a.substring(0, b.length()-1);
                String pswa11="";
                if(!b1.equals("")){
                	    pswa11 =b1+".00";
                }            
                ...
                if(!b1.equals("")){
             		try {
             			number1 = NumberFormat.getInstance().parse(pswa11);
             		} catch (ParseException e) {
             			// TODO Auto-generated catch block
             			e.printStackTrace();
             		} 
            		...
            		if(!number1.equals("")){
            			 gs1=numberFormat1.format(number1)+".00";
            		}else{
            			gs1="0.00";
            		}
                }else{
         		...
         		System.out.println("格式化金额"+gs1);
                passwordField.setText(gs1);                  
                String cs = passwordField.getText();
                char[] password11=cs.toCharArray();
                String pswa111 = new String(password11);
                System.out.println("删除后的的密码：" + pswa111);
            }  return passwordField;  
        }  
        ...
        public JTextField clickCommKey(String key) { 
            ...
            if (key != null) {  
            	...
            	int a22 = string.length();//通过length获取字符串长度，这里等于7

                if(!string.equals("")){
                  String a=  string.substring(0, string.length()-3);
                  System.out.println("截取"+a); 
                  System.out.println("添加"+a); 
                  //判断金额最大不超过999，999，999.00
                  if(a22<14){
                	  passwordField.setText( a+ key);  
                  }else{
                ...
                //将数字进行处理
                String ch1= passwordField.getText();
                char[] password11=ch1.toCharArray();
                String psws = new String(password11);
                java.lang.Number number = null;
        		try {
        			number = NumberFormat.getInstance().parse(psws);
        		} catch (ParseException e) {
        			// TODO Auto-generated catch block
        			e.printStackTrace();
        		}  
        		NumberFormat numberFormat1 = NumberFormat.getNumberInstance();
        		System.out.println("格式化金额"+numberFormat1.format(number));
        		String gs="";
        	
            		if(string.equals("")){
           			 gs=numberFormat1.format(number)+".00";
           		}else{
           			 gs=numberFormat1.format(number)+".00";
           		}        		
           		System.out.println("格式化金额最后"+gs);
                   passwordField.setText(gs);             
...
//                  this.add(key);  
//              } 
              KeyLable key1 = new KeyLable("确定", true, SoftKeyBoardPanel.this);  
              KeyLable key2 = new KeyLable("0", SoftKeyBoardPanel.this);  
//              KeyLable key3 = new KeyLable("删除", SoftKeyBoardPanel.this);  
              KeyLable key3 = new KeyLable("删除", true, SoftKeyBoardPanel.this);// 功能键，位置固定在最右  
...
//              key11.setPreferredSize(new Dimension(120, 50));  
    ...
    public static class KeyLable extends JLabel {  
        ...
        public KeyLable(String centerKey, String lowerLeftKey, boolean isFunctionKey, final ActionListener action) {  
            ...
            if (isFunctionKey) {// 这个变量主要是提高效率  
                if (centerKey.indexOf("Shift") >= 0) {  
                    ...
                    isBackSpace = true;  
                }else if (centerKey.indexOf("确") >= 0 || centerKey.indexOf("定") >= 0) {
                    isBackSpace1 = true; 
        ...
        public boolean isCommKey() {  
            return !isBackSpace && !isCapsLock && !isShift && !isBackSpace1;  
        }  
