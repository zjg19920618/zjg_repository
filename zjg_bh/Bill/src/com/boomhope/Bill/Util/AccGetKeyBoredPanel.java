...
public class AccGetKeyBoredPanel extends JPopupMenu {
	/**
	 * 
	 */
	static Logger logger = Logger.getLogger(AccGetKeyBoredPanel.class);
	...
	protected static SoftKeyBoardPanel accGetKeyBoredPanel;
	private static  Robot robot;
	
	public AccGetKeyBoredPanel(JTextField textField) {
		logger.info("进入账号键盘类");
	...
	public static class SoftKeyBoardPanel extends JPanel implements
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (keyLable.isBackSpace()) {
				clickBackSpace();
			} else if (keyLable.isConfirmSpace()) {
				clickConfirm();
		...
		public class RowPanel extends JPanel {
			...
			private void initRowPanel() {
				...
				if (rowType == RowType.first) {
					...
					// 功能键确认
					KeyLable key3 = new KeyLable("确认", true,SoftKeyBoardPanel.this);
		...
		public KeyLable(String centerKey, String lowerLeftKey,
			...
			if (isFunctionKey) {// 这个变量主要是提高效率
				if (centerKey.indexOf("删") >= 0 || centerKey.indexOf("除") >= 0) {
					isBackSpace = true;
				} else if (centerKey.indexOf("确") >= 0
						|| centerKey.indexOf("认") >= 0) {
					isConfirm = true;
				}
			}
