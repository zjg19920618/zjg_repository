...
public class BillChangeOpenHandInputBankCardPanel extends BaseTransPanelNew {
	
	static Logger logger = Logger.getLogger(BillChangeOpenHandInputBankCardPanel.class);
	 ...
	 * 初始化
	 */
	public BillChangeOpenHandInputBankCardPanel(){
		/*显示时间倒计时*/
		...
		voiceTimer = new Timer(BaseTransPanelNew.voiceSecond, new ActionListener() {          
            public void actionPerformed(ActionEvent e) {  
            	...
            	closeVoice();
            	if("1".equals(bcOpenBean.getSecondCardNocard())){
            		//两次录入不一致语音提示
	...
	private void nextstep(){
		...
		//跳转再次录入银行卡号
		openPanel(new BillChangeOpenHandSecondInputBankCardPanel());
	}
	...
	private void backstep(){
    	...
    	bcOpenBean.setSecondCardNocard("");//清空标识
		//跳转录入卡号选择页
		openPanel(new BillChangeOpenSelectMethodPanel());
	}
