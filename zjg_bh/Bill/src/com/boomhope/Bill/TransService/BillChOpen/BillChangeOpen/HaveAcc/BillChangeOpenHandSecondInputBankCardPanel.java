...
public class BillChangeOpenHandSecondInputBankCardPanel extends BaseTransPanelNew {
	
	static Logger logger = Logger.getLogger(BillChangeOpenHandSecondInputBankCardPanel.class);
	 ...
	 * 初始化
	 */
	public BillChangeOpenHandSecondInputBankCardPanel(){
		/*显示时间倒计时*/
	...
	private void nextstep(){
		...
		logger.info("开关当前状态"+on_off);
		on_off=false;
		bcOpenBean.setCardNo(str);
	
		if(bcOpenBean.getFirstCardNo().equals((bcOpenBean.getCardNo()) )){
			//校验银行卡信息
			openPanel(new BillChangeOpenCheckingBankcardPanel());
			
			
		}else{
			bcOpenBean.setSecondCardNocard("1");//两次卡号不一致标识
			//两次不一致重新录入银行卡
	...
	private void backstep(){
    	...
    	bcOpenBean.setSecondCardNocard("");//清空标识
		//跳转录入卡号选择页
		openPanel(new BillChangeOpenHandInputBankCardPanel());
	}
