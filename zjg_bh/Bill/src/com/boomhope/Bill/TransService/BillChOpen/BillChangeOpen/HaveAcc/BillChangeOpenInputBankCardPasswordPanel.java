 ...
 *
 */
public class BillChangeOpenInputBankCardPasswordPanel extends BaseTransPanelNew {
	
	static Logger logger = Logger.getLogger(BillChangeOpenInputBankCardPasswordPanel.class);
	 ...
	 * 初始化
	 */
	public BillChangeOpenInputBankCardPasswordPanel(){
		comp = this;
		...
		Thread thread  = new Thread("调用密码键盘线程"){
            ...
            public void run(){
				...
				try {
					...
					reqMap.put("CardNo",bcOpenBean.getCardNo());	//出账卡号
		    	...
		    	clearTimeText();//清空倒计时
				logger.info("密码获取成功!");
				bcOpenBean.setCardPwd(resMap.get("Password"));
				//与前置交互信息处理
				bcOpenAction.passWord(comp);
            }
        };
        thread.start();
	}
