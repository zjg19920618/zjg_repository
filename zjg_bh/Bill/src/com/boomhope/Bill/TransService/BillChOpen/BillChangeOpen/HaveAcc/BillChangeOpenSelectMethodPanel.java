 ...
 *
 */
public class BillChangeOpenSelectMethodPanel extends BaseTransPanelNew {
	private static final long serialVersionUID = 1L;
	private boolean on_off=true;//用于控制页面跳转的开关
	static Logger logger = Logger.getLogger(BillChangeOpenSelectMethodPanel.class);
	public BillChangeOpenSelectMethodPanel() {
		/* 显示时间倒计时 */
		...
		haveAcc.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) {
            	...
            	bcOpenBean.setInCardOrHandCard("1");//插卡标识
				openPanel(new BillChangeOpenInputBankCardPanel());
			}
		...
		noAcc.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) {
            	...
            	bcOpenBean.setInCardOrHandCard("2");//手输标识
				openPanel(new BillChangeOpenHandInputBankCardPanel());
			}
		...
		submitBtn.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) {
				...
				stopTimer(voiceTimer);//关闭语音
            	closeVoice();//关闭语音流
            	if("1".equals(bcOpenBean.getJudgeState())){
            		//提前支取返回拍照页面
            		openPanel(new BillChangeOpenCameraPanel());
            		
            	}else{
            		
            		if("1".equals(bcOpenBean.getAmtState())){
