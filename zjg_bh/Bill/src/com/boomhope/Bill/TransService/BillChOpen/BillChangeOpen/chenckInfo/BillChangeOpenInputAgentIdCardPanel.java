 ...
 *
 */
public class BillChangeOpenInputAgentIdCardPanel extends BaseTransPanelNew{
	Thread thread = null;
	static Logger logger = Logger.getLogger(BillChangeOpenInputAgentIdCardPanel.class);
	IdCardReadBean bean = null;
	SourceDataLine line = null;
	JButton button  = null;
	private static final long serialVersionUID = 1L;
	public BillChangeOpenInputAgentIdCardPanel() {
	...
	public void payInfoAgent(){
		bcOpenBean.setReadAgentIdNo(bean.getIdCode());//身份证号
		bcOpenBean.setReadAgentIdName(bean.getName());//姓名
		bcOpenBean.setAgentsex(bean.getSex());//性别
		bcOpenBean.setAgentBirthday(bean.getBirthday());//生日
		bcOpenBean.setAgentaddress(bean.getAddress());//地址
		bcOpenBean.setAgentqfjg(bean.getIssuingUnit());//签发机关
		bcOpenBean.setAgentEndDate(bean.getEndDate());//有效结束日期
	}
	...
	private void success(final Map map){
		...
		openPanel(new BillChangeOpenOutputAgentIdCardPanel(map));
	}
}
