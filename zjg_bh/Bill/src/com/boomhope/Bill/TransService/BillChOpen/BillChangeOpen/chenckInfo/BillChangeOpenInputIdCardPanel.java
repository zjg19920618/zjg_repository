 ...
 *
 */
public class BillChangeOpenInputIdCardPanel extends BaseTransPanelNew{
	Thread thread = null;
	static Logger logger = Logger.getLogger(BillChangeOpenInputIdCardPanel.class);
	IdCardReadBean bean = null;
	private static final long serialVersionUID = 1L;
	public BillChangeOpenInputIdCardPanel() {
		/*显示时间倒计时*/
	...
	public void getMyselfInfo(){
		bcOpenBean.setReadIdNo(bean.getIdCode());//身份证号
		bcOpenBean.setReadIdName(bean.getName());//姓名
		bcOpenBean.setSex(bean.getSex());//性别
		bcOpenBean.setBirthday(bean.getBirthday());//生日
		bcOpenBean.setAddress(bean.getAddress());//地址
		bcOpenBean.setQfjg(bean.getIssuingUnit());//签发机关
		bcOpenBean.setEndDate(bean.getEndDate());//有效结束日期
	}
	...
	private void success(final Map map){
		...
		openPanel(new BillChangeOpenOutputIdCardPanel(map));
	}
}
