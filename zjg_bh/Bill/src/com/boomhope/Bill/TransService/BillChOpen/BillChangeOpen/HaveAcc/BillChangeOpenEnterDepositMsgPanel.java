 ...
 *
 */
public class BillChangeOpenEnterDepositMsgPanel extends BaseTransPanelNew{
	
	static Logger logger = Logger.getLogger(BillChangeOpenEnterDepositMsgPanel.class);
	...
	public BillChangeOpenEnterDepositMsgPanel() {
		/*显示时间倒计时*/
    	...
    	closeVoice();//关语音流
		 ...
		 if((!"".equals(textField1.getText()) && textField1.getText()!=null )&&( !"".equals(textField2.getText()) && textField2.getText()!=null)){
			 logger.info("输入账号和凭证号查询存单信息");
				...
				bcOpenBean.setBillNo(textField1.getText());//凭证号
		
	    	if(StringUtils.isNotBlank(textField3.getText())){
	    		if("0008".equals(textField2.getText().substring(6,10))){
				    bcOpenBean.setSubAccNoCancel("3");//电子账号标识
				    logger.info("输入账号为电子子账号："+textField2.getText()+"-"+textField3.getText());
			    	logger.info("输入账号为电子子账号，匹配成功");
			    	bcOpenBean.setAccNo(textField2.getText()+"-"+textField3.getText());//账号-子账号
			    	bcOpenBean.setSubCardNo(textField2.getText());//账号
			    	bcOpenBean.setSubAccNo(textField3.getText());//子账号
				...
				bcOpenBean.setAccNo(textField2.getText());
