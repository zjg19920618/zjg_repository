 ...
 *
 */
public class TransPrintBankMimeographs extends BaseTransPanelNew{
	static Logger logger = Logger.getLogger(TransPrintBankMimeographs.class);
	 ...
	 * @打印页面
	 */
	public TransPrintBankMimeographs(final BillPrintBean transBean,final List<ICBankBean> bankBeans) {
		this.billPrintBean = transBean;
		this.bankBeans = bankBeans;
		logger.info("进入选择打印存单页面");
		...
		for (ICBankBean ICAccNo :this.bankBeans) {
			...
			if(ICAccNo.getICAccNo().equals(transBean.getAccNo())){
				...
				for (ICSubAccNo icSubAccNo : subAccNo) {
					...
					data[count][3]=icSubAccNo.getSavingCountMinStr();//存期
		...
		// 字体
		JLabel	label28 = new JLabel("请选择要打印的存单");
		...
		this.showTimeText(240);
		delayTimer = new Timer(240 * 1000, new ActionListener() {
		...
		voiceTimer = new Timer(voiceSecond, new ActionListener() {          
            public void actionPerformed(ActionEvent e) {  
            	...
            	excuteVoice("voice/checkprintbill.wav");
		...
		chckbxNewCheckBox.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if ((Boolean)data[3*transBean.getN()][11]){
					...
					transBean.getSelect().remove(icBank.getSubAccNo().get(i).getSubAccNo());
					icBank.getSubAccNo().get(i).setCheck(false);
					logger.debug("取消打印："+transBean.getSelect());
					}else{
						chckbxNewCheckBox.setIcon(new ImageIcon("pic/selected.png"));
						data[3*transBean.getN()][11]=true;
						transBean.getSelect().add(icBank.getSubAccNo().get(i).getSubAccNo());
						icBank.getSubAccNo().get(i).setCheck(true);
						logger.debug("增加打印："+transBean.getSelect());
					}
				}
		});
		}else{}
			...
			public void mouseClicked(MouseEvent e) {
				...
				if ((Boolean)data[3*transBean.getN()+1][11]){
					...
					logger.debug("取消打印："+transBean.getSelect());
						...
						transBean.getSelect().add(icBank.getSubAccNo().get(i).getSubAccNo());
						icBank.getSubAccNo().get(i).setCheck(true);
						logger.debug("选中打印："+transBean.getSelect());
					}
			}
		});
		}else{}
