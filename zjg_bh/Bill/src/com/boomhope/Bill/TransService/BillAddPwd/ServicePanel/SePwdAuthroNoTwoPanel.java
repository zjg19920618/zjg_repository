 ...
 *
 */
public class SePwdAuthroNoTwoPanel extends BaseTransPanelNew{
	static Logger logger = Logger.getLogger(SePwdAuthroNoTwoPanel.class);
	...
	public SePwdAuthroNoTwoPanel() {
		...
		if("1".equals(addPwdBean.getTellNoState())){
			textField.setText(addPwdBean.getTellNo1());
		}
		...
		upPageButton.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) {
				...
				if("1".equals(addPwdBean.getTellNoState())){
					...
					//清空授权标识
					addPwdBean.setTellNoState("");
					addPwdBean.setTellNo1("");
				}
				//跳转页面
	...
	private void nextStep() {
		new Thread() {
			...
			public void run() {
				...
				addPwdBean.setTellNo1(str1);
				addPwdBean.setTellNo2(str2);
				...
				if (str1 == null || "".equals(str1)) {//第一个柜员未录入提示
		    		...
		    		if(str2 == null || "".equals(str2)){//第二个柜员未录入
		    			...
		    			addPwdBean.setSupTellerNo(str1);
		    			...
		    			if(str1.equals(str2)){//两次柜员号不能一致
		    				...
		    				addPwdBean.setSupTellerNo(str2);
	...
	private void authorDeal( final String checkMod) {
		...
		if ("1".equals(checkMod)) {// 1 口令2 指纹
	    	...
	    	openPanel(new SetPwdAuthorPassPanel());
		} else {
