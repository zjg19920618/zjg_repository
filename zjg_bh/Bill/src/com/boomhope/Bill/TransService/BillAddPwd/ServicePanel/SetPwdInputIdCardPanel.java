...
public class SetPwdInputIdCardPanel extends BaseTransPanelNew{
	...
	Thread thread = null;
	static Logger logger = Logger.getLogger(SetPwdInputIdCardPanel.class);
	IdCardReadBean bean = null;
	public SetPwdInputIdCardPanel() {
		/*显示时间倒计时*/
		...
		thread  = new Thread("readIdCard"){
            ...
            public void run(){
        		...
        		try {
        			...
        			addPwdBean.getReqMCM001().setIntereturnmsg("身份证打开读卡器异常");
        		...
        		if("0".equals(status)){
        			...
        			try {
	        			...
	        			if(!flag){
	        				...
	        				addPwdBean.getReqMCM001().setIntereturnmsg("读取身份证信息失败");
	    					MakeMonitorInfo.makeInfos(addPwdBean.getReqMCM001());
	...
	public void getMyselfInfo(){
		addPwdBean.setReadIdNo(bean.getIdCode());//身份证号
		addPwdBean.setReadIdName(bean.getName());//姓名
		addPwdBean.setSex(bean.getSex());//性别
		addPwdBean.setBirthday(bean.getBirthday());//生日
		addPwdBean.setAddress(bean.getAddress());//地址
		addPwdBean.setQfjg(bean.getIssuingUnit());//签发机关
		addPwdBean.setEndDate(bean.getEndDate());//有效结束日期
	}
	...
	private void success(final Map map){
		...
		openPanel(new SetPwdOutputIdCardPanel(map));
	}
}
