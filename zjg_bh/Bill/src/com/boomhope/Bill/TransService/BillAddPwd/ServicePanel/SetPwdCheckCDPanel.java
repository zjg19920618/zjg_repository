 ...
 *
 */
public class SetPwdCheckCDPanel extends BaseTransPanelNew{


	static Logger logger = Logger.getLogger(SetPwdCheckCDPanel.class);
	private static final long serialVersionUID = 1L;
	private String billImgPath;//扫描图片路径
	public SetPwdCheckCDPanel(){
		...
		new Thread(){
			public void run(){
				...
				if("999999".equals(check.get("resCode"))){//存单信息获取失败，进入服务终止页
					...
					openPanel(new SetPwdcheckCDFailPanel(check.get("errMsg")));
					return;
				}
				...
				if("999999".equals(ocrCheck.get("resCode"))){//OCR识别不通过,进入手动输入选择页
	        		...
	        		openPanel(new SetPwdcheckCDFailPanel(ocrCheck.get("errMsg")));
					return;
					
				}else if("999998".equals(ocrCheck.get("resCode"))){//OCR识别账号与选择密码增设方式不匹配，弹框重新选择密码增设
				...
				if("999999".equals(judgeBillMsg.get("resCode"))){
					...
					return;
				}else if("999998".equals(judgeBillMsg.get("resCode"))){//OCR识别账号不一致，弹框重新选择密码增设页面
	        		...
	        		openPanel(new SetPwdcheckCDFailPanel(judgeBillMsg.get("errMsg")));
					return;
				}
				
				addPwdBean.setCheckStatus("1");//标记位，存在数据库中，标记这个存单是自动扫描成功的一单
				//全部成功，进入存单展现页
				clearTimeText();//清空倒计时
				stopTimer(voiceTimer);//关闭语音
            	closeVoice();//关闭语音流
	...
	public Map<String,String> jwCheck(){
		...
		try{
			...
			if(fc == null || "".equals(fc)){
				...
				String tmp = Property.BH_TMP + "\\"+DateUtil.getNowDate("yyyyMMdd")+"\\" +addPwdBean.getFid()+"\\";
				// --------------------------fc上传----------------
				// 源目录
				...
				addPwdBean.setBillPath_fc(tmp+from_fc.getName());
				addPwdBean.setBillPath_rc(tmp+from_rc.getName());	
			}
		...
		try{
			...
			addPwdBean.setJwResult(String.valueOf(i));
			//上送鉴伪结果
			try {
				
				IntefaceSendMsg.tms0007(addPwdBean);
			} catch (Exception e) {
	...
	public Map<String,String> OCRCheck(){
		...
		//判断识别为账号/卡号-子账号
				try {
					...
					addPwdBean.setBillNo(billNo);//凭证号
				      ...
				      if(accNo.contains("-")){
				    	 if("0008".equals(accNo.substring(6,10))){
						    ...
						    }else if( !"0008".equals(accNo.substring(6,10))){
							...
							maps.put("errMsg", "该存单非电子账号、普通账号、卡下子账号，请选择其他增密方式");
							return maps;
						}
	...
	public boolean checkBillMsg(){
		try {
			...
			if(!"000000".equals(inter07601.get("resCode"))){
				...
				openPanel(new SetPwdcheckCDFailPanel((String)inter07601.get("errMsg")));
				return false;
			}
	...
	public Map<String, String> judgeBillMsg(){
		Map<String,String> map = new HashMap<String,String>();
		//判断该存单是否有存单密码，如果存在密码，且机构号也不相等，则拒单
		try {
...
//			}
	...
	public void checkResult(String msg){
		openConfirmDialog(msg +"。是：重新选择增设密码页面;否：退出业务;");
		confirmDialog.YseButten.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseReleased(MouseEvent e) {
				
				logger.info("进入重新选择密码增设页面");
		...
		confirmDialog.Nobutton.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) {
				...
				stopTimer(voiceTimer);//关闭语音
            	closeVoice();//关闭语音流
				openPanel(new SetPwdOutputCDPanel());
			}
		});
	}
