...
public class SetPwdCameraPanel extends BaseTransPanelNew{
	static Logger logger = Logger.getLogger(SetPwdCameraPanel.class);
	...
	public SetPwdCameraPanel() {
		...
		if(addPwdBean.getCameraCount()==null ||"".equals(addPwdBean.getCameraCount())){
			addPwdBean.setCameraCount("0");
		}
		...
		submitBtn.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) {
				...
				if("1".equals(addPwdBean.getTellerIsChecked())){
					logger.info("返回身份信息页面");
					openPanel(new SetPwdCheckPohotsPanel());
					
				}else{
					//跳转回上一页面
					logger.info("返回插入身份证页面");
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					addPwdBean.getReqMCM001().setIntereturnmsg("摄像头打开失败");
	...
	public void pz() {
		try {
        	...
        	closeVoice();//关闭语音流
			openPanel(new SetPwdFaceCheckFailPanel("拍照失败，请联系大堂经理", ""));
		}		
	}
	// 保存图片
		...
		public void nextStep(){
			...
			stopTimer(voiceTimer);
			player.close();	
			if("1".equals(addPwdBean.getCameraCount())){
				//重新拍照跳转过来
			...
			//页面跳转
			openPanel(new SetPwdFaceCheckPanel());
		}
}
