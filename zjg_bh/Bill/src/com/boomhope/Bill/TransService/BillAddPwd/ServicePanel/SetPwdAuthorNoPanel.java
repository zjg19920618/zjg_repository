 ...
 *
 */
public class SetPwdAuthorNoPanel extends BaseTransPanelNew {

static Logger logger = Logger.getLogger(SetPwdAuthorNoPanel.class);
private static final long serialVersionUID = 1L;
	...
	public SetPwdAuthorNoPanel(){
		...
		/* 倒计时打开语音 */
		if(addPwdBean.getTellerIsChecked()==null || "".equals(addPwdBean.getTellerIsChecked())){
		...
		//主管授权
		JLabel jlabel = new JLabel("请确认业务信息后授权");
		...
		upPageButton.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) {
				...
				openPanel(new SetPwdCheckBillQZPanel());//确认用户信息 签字
			}
		});
		add(upPageButton);
	...
	private void showKeyBord() {
		if(!"2".equals(addPwdBean.getTellerIsChecked())){
	...
	private void authorDeal( final String checkMod) {
		...
		if ("1".equals(checkMod)) {// 1 口令2 指纹
			...
			openPanel(new SetPwdAuthorNoPassPanel());
		} else {
			logger.info("进入指纹授权"+checkMod);			
			openPanel(new SetPwdAuthroNoPrintPanel());
		}
