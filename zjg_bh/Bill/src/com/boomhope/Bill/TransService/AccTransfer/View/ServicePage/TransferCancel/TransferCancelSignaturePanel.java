 ...
 *
 */
public class TransferCancelSignaturePanel extends BaseTransPanelNew {
	
	static Logger logger = Logger.getLogger(TransferCancelSignaturePanel.class);
	/*倒计时时间*/
	JLabel lblNewLabel;
	JLabel promptLabel;
	...
	public TransferCancelSignaturePanel(final PublicAccTransferBean transferBean){
	...
	private void okNext(){
		...
		try {
			screen.Screenshot(515, 280, panel.getWidth(), panel.getHeight(), Property.SIGNATURE_PATH_CANCEL);
	...
	public void nextStep(){
		delayTimer.stop();
		transferMoneyBean.setIsSign("Y");
		openPanel(new TransferCancelSignPanel(transferMoneyBean));
	}
