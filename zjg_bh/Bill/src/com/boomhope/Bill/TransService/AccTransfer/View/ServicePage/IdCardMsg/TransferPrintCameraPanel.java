...
public class TransferPrintCameraPanel extends BaseTransPanelNew {
	...
	public TransferPrintCameraPanel(final PublicAccTransferBean transferBean) {
		logger.info("进入拍照页面");
		...
		if(transferMoneyBean.getCameraCount()==null ||"".equals(transferMoneyBean.getCameraCount())){
			transferMoneyBean.setCameraCount("0");
		}
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					logger.error("拍照失败："+e);
					player.close();
					openPanel(new TransferFaceCheckFail("拍照失败，请联系大堂经理", "",transferMoneyBean));
				}
	...
	public void nextStep(){
		...
		player.close();
		//页面跳转
		openPanel(new TransAccFaceCheckPanel(transferMoneyBean));
	}
