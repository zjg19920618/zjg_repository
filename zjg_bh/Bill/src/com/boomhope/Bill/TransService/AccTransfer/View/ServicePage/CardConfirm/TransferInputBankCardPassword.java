 ...
 *
 */
public class TransferInputBankCardPassword extends BaseTransPanelNew {
	
	static Logger logger = Logger.getLogger(TransferInputBankCardPassword.class);
	 ...
	 * 初始化
	 */
	public TransferInputBankCardPassword(final PublicAccTransferBean transferBean){
		...
		Thread thread  = new Thread("调用密码键盘线程"){
            ...
            public void run(){
				...
				try {
					...
					reqMap.put("CardNo",transferBean.getChuZhangCardNo());	//出账卡号
				...
				//与前置交互信息处理
				logger.info("进入汇划转账密码验证");
				transferAction.passWord(comp,transferBean);//个人转账	
            }
        };
        thread.start();
	}
