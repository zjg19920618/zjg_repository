...
import com.boomhope.Bill.Framework.BaseTransPanel;
...
public class AccInputBankCardPassword extends BaseTransPanelNew {
	
	static Logger logger = Logger.getLogger(AccInputBankCardPassword.class);
	 ...
	 * 初始化
	 */
	public AccInputBankCardPassword(final Map params){
	 ...
	 * @param errLable
	 */
	private void inputPassword(final Component comp,final Map params) {
		...
		Thread thread  = new Thread("调用密码键盘线程"){
            ...
            public void run(){
				try {
					...
					reqMap.put("CardNo", (String)params.get("cardNo"));	// 存单号
					...
					if (resMap.get("ResCode").endsWith("S")){
						...
						params.put("passWord", resMap.get("Password"));
