 ...
 * @author zjg
 */
public class AccJxcInputPanel extends BaseTransPanelNew {
	private static final long serialVersionUID = 1L;
	static Logger logger = Logger.getLogger(AccJxcInputPanel.class);
	...
	public AccJxcInputPanel(Map<Object, Object> params) {
		...
		lblNewLabel_21.setBounds(590, 329, 103, 40);
		...
		button11_1.setBounds(693, 329, 269, 40);
		...
		// 存入
		JLabel label_2 = new JLabel("<html><p>每月固定</p><p>存入金额 :</p></html>");
		label_2.setFont(new Font("微软雅黑", Font.PLAIN, 24));
		label_2.setForeground(Color.decode("#333333"));
		label_2.setBounds(69, 436, 130, 90);
		this.showJpanel.add(label_2);

		// 循环出金额和所对应的产品代码(积享存有固定存入金额)
		num = new String[] { "1000", "2000", "3000", "4000", "5000", "10000" };
		int x1 = 194;
		buttons = new JLabel[num.length];
		for (int i = 0; i < num.length; i++) {
			final int a = i;
			// 循环按钮
			buttons[i] = new JLabel(num[i], new ImageIcon(
					"pic/newPic/inputinfo.png"),0);
			buttons[i].setBounds(x1, 465, 90,40);
						...
						public void mouseReleased(MouseEvent e) {
							...
							for (int j = 0; j < buttons.length; j++) {
								...
								if (buttons[a].equals(buttons[j])) {
									...
									isbutton = true;
									button(num[a]+".00",list.get(0).getProductCode(),list.get(0).getProductCodeName());
									jslx();
								} else {
									buttons[j].setIcon(new ImageIcon(
		...
		lblNewLabel_3.setBounds(69, 534, 150, 40);
	...
	/**button监听事件：获取数据*/
	public void button(String moneys,String productCode,String productCodeName){
		logger.info("进入获取数据方法");
		
		if(moneys!=null&&productCode!=null&&productCodeName!=null){
			accBean.setMoney(moneys);//产品金额
	...
	public boolean poss(){
		...
		//到期时间计算
//		try {
//			if(accBean.getMonthsUpper()!=null && accBean.getMonthsUpper()!=""){
//				
//				DateTool dt = new DateTool();
//				if(accBean.getMonthsUpper().substring(accBean.getMonthsUpper().length()-1, accBean.getMonthsUpper().length()).equals("D")){
//					String abc = dt.nDaysAfterOneDateString(accBean.getSvrDate(),Integer.parseInt(accBean.getMonthsUpper().replace("D", "")));
//					accBean.setEndTime(abc);
//					logger.info("到期时间"+accBean.getEndTime());
//				}else{
//					String abc = dt.nMonthsAfterOneString(accBean.getSvrDate(), accBean.getMonthsUpper());
//					accBean.setEndTime(abc);
//					logger.info("到期时间"+accBean.getEndTime());
//				}
//			}else{
...
//		}
		...
		//存期转换数字、单位
		try {
			String str=accBean.getMonthsUpper().toUpperCase();
