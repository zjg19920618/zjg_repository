 ...
 *
 */
public class AccCancelEnterDepositMsgPanel extends BaseTransPanelNew{
	
	static Logger logger = Logger.getLogger(AccCancelEnterDepositMsgPanel.class);
	...
	public AccCancelEnterDepositMsgPanel() {
		/*显示时间倒计时*/
	...
	public void nextStep(){
		accCancelBean.setBillNo(textField1.getText());
		...
		accCancelBean.setBillNo(textField1.getText());//凭证号
		accCancelBean.getReqMCM001().setRvouchertype("102");
		accCancelBean.getReqMCM001().setRvoucherno(textField1.getText());
		
		String substring = textField2.getText().substring(6,10);
		if("002".equals(accCancelBean.getAccCancelType())){
			
			//选择电子子账户销户
			logger.info("选择电子子账户销户");
			...
			if(StringUtils.isNotBlank(textField3.getText())){
				...
				if("0008".equals(textField2.getText().substring(6,10))){
					...
					accCancelBean.getReqMCM001().setAccount(textField2.getText()+"-"+textField3.getText());
					accCancelBean.setAccNo(textField2.getText()+"-"+textField3.getText());//账号-子账号
					accCancelBean.setSubCardNo(textField2.getText());//账号
					accCancelBean.setSubAccNo(textField3.getText());//子账号
					accCancelBean.setCardNo(textField2.getText());//转入的电子卡号
					accCancelBean.getReqMCM001().setToaccount(textField2.getText());
					
					clearTimeText();//清空倒计时
					stopTimer(voiceTimer);//关语音
			    	closeVoice();//关语音流
					//进入存单信息查询页
			    	openPanel(new AccCancelCheckTFSecondPanel());
					
				}else{
					
					logger.info("输入账号非电子子账号，匹配失败");
					on_off=true;
					openMistakeDialog("抱歉，您选择的销户转存方式不支持银行卡子账户");
					return;
				}
				...
				logger.info("输入账号非电子子账号，匹配失败");
				on_off=true;
				openMistakeDialog("抱歉，您选择的销户转存方式不支持普通账户");
				return;
			}
		}else{
			
			//选择普通账户、卡下子账户销户转银行卡/转唐行宝
			logger.info("选择普通账户、卡下子账户销户");
			
			if(!StringUtils.isNotBlank(textField3.getText())){//识别为普通账号
				
				logger.info("输入账号为普通账号，匹配成功");
				accCancelBean.setSubAccNoCancel("1");//普通账号标识
				accCancelBean.setAccNo(textField2.getText());
				accCancelBean.getReqMCM001().setAccount(textField2.getText());
				...
				accCancelBean.setSubCardNo(textField2.getText());//账号
				accCancelBean.setSubAccNo(textField3.getText());//子账号
				accCancelBean.setCardNo(textField2.getText());//转入卡号
				accCancelBean.getReqMCM001().setToaccount(textField2.getText());
				...
				logger.info("输入账号非普通账号、卡下子账号，匹配失败");
				on_off=true;
				openMistakeDialog("抱歉，您选择的销户转存方式不支持电子子账户");
				return;
			}
		}
