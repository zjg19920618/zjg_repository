 ...
 *
 */
public class AccCancelBankEMsgPanel extends BaseTransPanelNew{
	private static final long serialVersionUID = 1L;
	static Logger logger = Logger.getLogger(AccCancelBankEMsgPanel.class);
	...
	public AccCancelBankEMsgPanel() {
		...
		JLabel labelName = new JLabel("请选择电子子账户信息");
	...
	public void initPanelShow(){
		...
		EAccInfoBean subBean = (EAccInfoBean)accCancelBean.getImportMap().get("E_ACC_MSG").get(nowPage-1);
		cardNo = subBean.geteCardNo();//卡号
		subAccNo = subBean.geteSubAccNo();//子账号
		openInstNo = subBean.geteOpenInstNo();//开户机构
		openDate = subBean.geteOpenDate();//开户日期
		endIntDate = subBean.geteEndDate();//到期日期
		openATM = subBean.geteOpenATM();//开户金额
		productName = subBean.geteProductName();//产品名称
		depTerm = subBean.geteDepTerm();//存期
		openRate = subBean.geteOpenRate();//利率
		balance = subBean.geteBalance();//结存额
		accNoState = subBean.geteAccNoState();//存单状态
		...
		//总页数
		JLabel label19 = new JLabel(String.valueOf(accCancelBean.getImportMap().get("E_ACC_MSG").size()));
	...
	public void confirm(){
		...
		accCancelBean.getReqMCM001().setToaccount(cardNo);
		...
		if(endDate > nowDate){//提前支取
			...
			confirmDialog.Nobutton.addMouseListener(new MouseAdapter() {
				...
				public void mouseReleased(MouseEvent e) {
					closeDialog(confirmDialog);
					//退出弹框
					openPanel(new AccCancelBankEMsgPanel());//停留在当前页
				}
			});	
	...
	public void  nextStep(){
		clearTimeText();//清空倒计时
		if(nowPage>=accCancelBean.getePages()){
			openMistakeDialog("已经没有更多信息了");
			return;
		}
		nowPage++;
