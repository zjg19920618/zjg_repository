 ...
 *
 */
public class AccCancelCheckTFPanel extends BaseTransPanelNew {
	static Logger logger = Logger.getLogger(AccCancelCheckTFPanel.class);
	private static final long serialVersionUID = 1L;
	private String billImgPath;//扫描图片路径
	
	public AccCancelCheckTFPanel() {
		/*显示时间倒计时*/
		...
		new Thread(){
			public void run(){
				...
				if("999999".equals(check.get("resCode"))){//存单信息获取失败，进入服务终止页
					...
					openPanel(new AccCancelCheckTFFailPanel(check.get("errMsg")));
					return;
				}
				...
				if("999999".equals(ocrCheck.get("resCode"))){//OCR识别不通过,进入手动输入选择页
					...
					openPanel(new AccCancelCheckTFFailPanel(ocrCheck.get("errMsg")));
					return;
					
				}else if("999998".equals(ocrCheck.get("resCode"))){//OCR识别账号与选择销户方式不匹配，弹框重新选择销户方式
				...
				if("999999".equals(judgeBillMsg.get("resCode"))){
					...
					return;
				}else if("999998".equals(judgeBillMsg.get("resCode"))){//OCR识别账号不一致，弹框重新选择销户方式
					...
					openPanel(new AccCancelCheckTFFailPanel(judgeBillMsg.get("errMsg")));
					return;
				}
				
				accCancelBean.setCheckStatus("1");//标记位，存在数据库中，标记这个存单是自动扫描成功的一单
				//全部成功，进入存单展现页
				clearTimeText();//清空倒计时
				stopTimer(voiceTimer);//关闭语音
            	closeVoice();//关闭语音流
	...
	public Map<String,String> jwCheck(){
		...
		try{
			...
			accCancelBean.setJwResult(String.valueOf(i));
			//上送鉴伪结果
			try {
				
				IntefaceSendMsg.tms0007(accCancelBean);
			} catch (Exception e) {
			...
			if(i != 0){
				...
				map.put("errMsg", "存单鉴伪结果不通过,请重新放入存单或手工输入");
				return map;
			}
	...
	public Map<String,String> OCRCheck(){
		...
		try {
			...
			if(accNo == null || "".equals(accNo.trim())){
				...
				maps.put("errMsg", "账号识别失败，请重新放入存单或手工输入");
				return maps;
			}
			if(billNo == null || "".equals(billNo.trim())){
				
				maps.put("resCode", "999999");
				maps.put("errMsg", "凭证号识别失败，请重新放入存单或手工输入");
				return maps;
			}
			accNo = accNo.trim().replace("\r\n", "");
			billNo = billNo.trim().replace("\r\n", "");
			accCancelBean.getReqMCM001().setAccount(accNo);
			accCancelBean.getReqMCM001().setRvouchertype("102");
			accCancelBean.getReqMCM001().setRvoucherno(billNo);
			
		} catch (Exception e) {
			logger.error("OCR识别失败"+e);
			maps.put("resCode", "999999");
			maps.put("errMsg", "票面信息识别失败，请重新放入存单或手工输入");
			return maps;
		}
		...
		try {
			...
			accCancelBean.setBillType("");//凭证种类
			accCancelBean.setBillNo(billNo);//凭证号
			
			if("002".equals(accCancelBean.getAccCancelType())){//选择电子子账户销户
				logger.debug("选择电子子账户销户");
				if(accNo.contains("-") && "0008".equals(accNo.substring(6,10))){
					
					logger.debug("识别账号为电子子账号，匹配成功");
					String[] split = accNo.split("-");
					accCancelBean.setAccNo(accNo);//账号-子账号
					accCancelBean.setSubCardNo(split[0]);//账号
					accCancelBean.setSubAccNo(split[1]);//子账号
					accCancelBean.setCardNo(split[0]);//转入的电子卡号
					accCancelBean.getReqMCM001().setToaccount(split[0]);
					
				}else{
					//识别结果非电子子账户则报错
					logger.debug("识别账号非电子子账号，匹配失败");
					maps.put("resCode", "999998");
					maps.put("errMsg", "该存单非电子子账户，请选择其他销户方式");
					return maps;
				}
			}else{
				
				//选择普通账户、卡下子账户销户转银行卡/转唐行宝
				logger.debug("选择普通账户、卡下子账户销户");
				
				if(!accNo.contains("-")){//识别为普通账号
					
					logger.debug("识别账号为普通账号，匹配成功");
					accCancelBean.setSubAccNoCancel("1");//普通账号标识
					accCancelBean.setAccNo(accNo);//账号
				
				}else if(!"0008".equals(accNo.substring(6,10))){//识别为卡下子账号
					
					logger.debug("识别账号为卡下子账号，匹配成功");
					accCancelBean.setSubAccNoCancel("0");//卡下子账户标识
					accCancelBean.setBillType("102");//凭证种类
					String[] split = accNo.split("-");
					accCancelBean.setAccNo(accNo);//账号-子账号
					accCancelBean.setSubCardNo(split[0]);//账号
					accCancelBean.setSubAccNo(split[1]);//子账号
					accCancelBean.setCardNo(split[0]);//转入卡号
					accCancelBean.getReqMCM001().setToaccount(split[0]);

					
				}else{//识别非普通账号、卡下子账号报错
					
					logger.debug("识别账号非普通账号、卡下子账号，匹配失败");
					maps.put("resCode", "999998");
					maps.put("errMsg", "该存单非普通账号、卡下子账号，请选择其他销户方式");
					return maps;
				}
			}
		}catch (Exception e) {
	...
	public boolean checkBillMsg(){
		try {
			...
			if(!"000000".equals(inter07601.get("resCode"))){
				...
				openPanel(new AccCancelCheckTFFailPanel((String)inter07601.get("errMsg")));
				return false;
			}
...
//			}
		...
		try {
			...
			if(!q_certNo.equals(accCancelBean.getBillNo().replace("\r\n", ""))){
				...
				map.put("resCode", "999998");
				map.put("errMsg", "票面信息识别失败，请重新放入存单或手工输入");
				return map;
			}
