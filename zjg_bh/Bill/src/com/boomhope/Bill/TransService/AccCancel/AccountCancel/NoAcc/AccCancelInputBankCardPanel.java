...
public class AccCancelInputBankCardPanel extends BaseTransPanelNew{
	 ...
	 * 初始化
	 */
	public AccCancelInputBankCardPanel(final Map params){
		this.comp=this;
            ...
            public void run(){
            	try {
        			...
        			if("0".equals(status)){
        				...
        				//查看是否是唐山银行卡
        				String cardNo = read.getIcBankCode();
        				...
        				//调用赋值方法
        				if(!payForBean(read,accCancelBean)){
        					logger.info("获取55域卡信息失败");
        					return;
        				}
        				...
        				//进入输入密码页
        				openPanel(new AccCancelInputBankCardPasswordPanel());
        			}else{
        				if("3".equals(status)){
        					//超时
	 ...
	 * 银行卡信息赋值
	 */
	public boolean payForBean(ICBankReadBean55 read,final PublicAccCancelBean accCancelBean){
