...
import com.boomhope.Bill.TransService.AccCancel.AccountCancel.HaveAcc.AccCancelInputDepositPanels;
 ...
 *
 */
public class AccCancelAuthorPassJwPanel extends BaseTransPanelNew{

	static Logger logger = Logger.getLogger(AccCancelAuthorPassJwPanel.class);
	 ...
	 * 初始化
	 */
	public AccCancelAuthorPassJwPanel() {
		...
		voiceTimer = new Timer(voiceSecond, new ActionListener() {
			public void actionPerformed(ActionEvent e) {
            	...
            	closeVoice();//关语音流
				excuteVoice("voice/caozuoyuan.wav");
		...
		/* 标题信息 */
		JLabel titleLabel = new JLabel("请录入操作员密码:");
		...
		errInfo = new JLabel("提示：请录入操作员密码!");
		...
		new Thread(){
			...
			public void run() {
				...
				try {
					...
					accCancelBean.getReqMCM001().setReqBefor("77017");
					...
					closeDialog(prossDialog);
					//跳入下一页
					if(accCancelBean.getTellNo1()!=null && !"".equals(accCancelBean.getTellNo1())){
						
						if (accCancelBean.getTellNo2()==null || "".equals(accCancelBean.getTellNo2())){
							//第一次授权标识
							accCancelBean.setTellNoState("1");
							//再次返回第二次授权
							openPanel(new AccCancelAuthorNoJwPanel());
							...
							//第二次授权标识
							accCancelBean.setTellNoState("2");
							//重新放入存单
							openPanel(new AccCancelInputDepositPanels());
						}
					}
					...
					accCancelBean.getReqMCM001().setIntereturnmsg("调用77017接口失败");
