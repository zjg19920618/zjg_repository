 ...
 *
 */
public class AccCancelInputBankCardPasswordPanel extends BaseTransPanelNew {
	
	static Logger logger = Logger.getLogger(AccCancelInputBankCardPasswordPanel.class);
	 ...
	 * 初始化
	 */
	public AccCancelInputBankCardPasswordPanel(){
		comp = this;
		...
		Thread thread  = new Thread("调用密码键盘线程"){
            ...
            public void run(){
				...
				try {
					...
					reqMap.put("CardNo",accCancelBean.getCardNo());	//出账卡号
		    	...
		    	clearTimeText();//清空倒计时
				logger.info("密码获取成功!");
				accCancelBean.setCardPwd(resMap.get("Password"));
				//与前置交互信息处理
