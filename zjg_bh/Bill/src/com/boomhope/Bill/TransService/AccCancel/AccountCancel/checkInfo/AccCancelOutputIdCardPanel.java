 ...
 * 
 */
public class AccCancelOutputIdCardPanel extends BaseTransPanelNew {
	final int voiceSecond = 500;
	UtilVoice utilVoice = null;
	private static final long serialVersionUID = 1L;
	static Logger logger = Logger.getLogger(AccCancelOutputIdCardPanel.class);
	private Component comp; 
	private boolean on_off=true;//用于控制页面跳转的开关

	public AccCancelOutputIdCardPanel(final Map map) {
		/*显示时间倒计时*/
	...
	public void nextStep(final Map map) {
		new Thread(new Runnable() {
			...
			public void run() {
				...
				clearTimeText();
				stopTimer(voiceTimer);
				openPanel(new AccCancelMePoliceCheckPanel());
			}
		}).start();
	}
	...
	private boolean checkIdCard(Map map) {
		...
		try {
			...
			if(!accCancelBean.getBillIdNo().trim().equals(accCancelBean.getReadIdNo()) || !accCancelBean.getBillIdName().trim().equals(accCancelBean.getReadIdName())){
				...
				confirmDialog.YseButten.addMouseListener(new MouseAdapter() {
					...
					public void mouseReleased(MouseEvent e) {
						...
						//重新插入身份证
						openPanel(new AccCancelInputIdCardPanel());//插入身份证页
					}
