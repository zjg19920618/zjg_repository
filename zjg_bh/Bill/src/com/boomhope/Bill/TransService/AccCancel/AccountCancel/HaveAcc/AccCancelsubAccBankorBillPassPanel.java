...
public class AccCancelsubAccBankorBillPassPanel extends BaseTransPanelNew {
	
	static Logger logger = Logger.getLogger(AccCancelsubAccBankorBillPassPanel.class);
	 ...
	 * 初始化
	 */
	public AccCancelsubAccBankorBillPassPanel(){
            ...
            public void actionPerformed(ActionEvent e) {  
            	...
            	if("0".equals(accCancelBean.getHaveAcc())){
        			...
        			if("002".equals(accCancelBean.getAccCancelType())){
        				...
        				excuteVoice("voice/EbankPass.wav");
        				
        			}else if("0".equals(accCancelBean.getSubAccNoCancel())){
        				
        				excuteVoice("voice/bankPass.wav");
		...
		inputPassword(comp,accCancelBean);
	}
	 ...
	 * @param errLable
	 */
	private void inputPassword(final Component comp,final PublicAccCancelBean transferBean) {
		Thread thread  = new Thread("调用存单密码键盘线程"){
            ...
            public void run(){
				...
				try {
					...
					if("002".equals(transferBean.getAccCancelType())){
						
						reqMap.put("CardNo",transferBean.getCardNo());	//电子账号
						
					}else if("0".equals(transferBean.getSubAccNoCancel())){
						
						reqMap.put("CardNo",transferBean.getCardNo());	//银行卡卡号
						
					}else{
						
						reqMap.put("CardNo",transferBean.getAccNo());	//存单账号
					}
				...
				if("002".equals(transferBean.getAccCancelType())){
					...
					transferBean.setBillPass(resMap.get("Password"));//电子账号密码
					
				}else if("0".equals(transferBean.getSubAccNoCancel())){
					
					transferBean.setCardPwd(resMap.get("Password"));//卡下子账号银行卡密码
					
				}else{
					
					transferBean.setBillPass(resMap.get("Password"));//存单密码
				}
				//与前置交互信息处理
				logger.info("进入销户密码校验");
				
				noAccAction.checkPass(comp);
