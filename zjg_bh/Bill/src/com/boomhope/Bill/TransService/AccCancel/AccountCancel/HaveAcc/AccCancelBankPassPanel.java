...
public class AccCancelBankPassPanel extends BaseTransPanelNew {
	
	static Logger logger = Logger.getLogger(AccCancelBankPassPanel.class);
	 ...
	 * 初始化
	 */
	public AccCancelBankPassPanel(){
		logger.info("普通账户转唐行宝,进入输入银行卡密码页面");
		//将当前页面传入流程控制进行操作
		...
		/*提示信息*/
		JLabel titleLabel = new JLabel("转入唐行宝，请录入银行卡密码");
	...
	private void inputPassword(final Component comp,final PublicAccCancelBean transferBean) {
		Thread thread  = new Thread("调用银行卡密码键盘线程"){
            @Override
            public void run(){
				...
				try {
					...
					reqMap.put("CardNo",transferBean.getCardNo());	//卡号
		    	...
		    	stopTimer(voiceTimer);//关闭语音
		    	clearTimeText();//清空倒计时
				logger.info("银行卡密码获取成功!");
				transferBean.setCardPwd(resMap.get("Password"));
				//与前置交互信息处理
				logger.info("进入银行卡密码查询");
				//进入唐行宝查询
				noAccAction.txbCheck(comp);
