 ...
 *
 */
public class MoneyBoxEnteringSHRYCJPanel extends BaseTransPanelNew{
	private static final long serialVersionUID = 1L;
	static Logger logger = Logger.getLogger(MoneyBoxEnteringSHRYCJPanel.class);
	JLabel promptLabel;
	...
	public MoneyBoxEnteringSHRYCJPanel(final PublicCashOpenBean transBean) {
		...
		String money = String.valueOf(transBean.getMoney());
		money = money.substring(0,money.length()-4);
		JButton button2 = new JButton(money+"万元", new ImageIcon(
				"pic/newPic/inputinfo.png"));
		button2.setFont(new Font("微软雅黑", Font.PLAIN, 20));
		button2.setHorizontalTextPosition(SwingConstants.CENTER);
		button2.setBounds(182, 430, 244, 40);
		button2.setOpaque(false);// 设置控件是否透明，true为不透明，false为透明
		button2.setContentAreaFilled(false);// 设置图片填满按钮所在的区域
		button2.setFocusPainted(true);// 设置这个按钮是不是获得焦点
		button2.setBorderPainted(false);// 设置是否绘制边框
		button2.setBorder(null);// 设置边框
		button2.setIcon(new ImageIcon("pic/xfk.png"));
		...
		lblNewLabel_8.setBounds(685, 295, 132, 40);
	...
	public void backTrans(PublicCashOpenBean transBean) {
		clearTimeText();
		openPanel(new MoneyBoxCatalogSHRYCPanel(transBean));
...
//		playVoice("voice/optatuodep.wav");
	...
	public void okTrans(PublicCashOpenBean transBean){
		 ...
		 * 当用户全部选择之后进入下一个页面
		 * */
		if(/*isOption()==true&&money()==true&&*/ispaypass()==true&&isautos()==true){
//			this.a();
