 ...
 *
 */
public class MoneyBoxEnteringLDCPanel extends BaseTransPanelNew {
	private static final long serialVersionUID = 1L;
	static Logger logger = Logger.getLogger(MoneyBoxEnteringLDCPanel.class);
	...
	public MoneyBoxEnteringLDCPanel(final PublicCashOpenBean transBean) {
...
//		/* 上一页 */
//		JButton backButton1 = new JButton(new ImageIcon("pic/tk.png"));
...
//		backButton1.setLocation(780, 638);
//
//		this.add(backButton1);
	...
	this.add(label_1);
		...
		lblNewLabel_8.setBounds(182, 425, 300, 40);
...
//	}
	 ...
	 * 创建利息线程
	 */
	public void startInte(final PublicCashOpenBean transBean) {
		...
		if (ispaypass() == true && isautos() == true) {
			...
			if(ispaykey==false){
				...
				openPanel(new MoneyBoxWithCloseTopayPanel(transBean));
					...
					String customer_identification = this.transBean.getImportMap().get("customer_identification");
//					int money = Integer.parseInt(transBean.getOrderAmount().replace(".00", ""));
					if(transBean.getMoney()>=50000.00 || "yes".equals(customer_identification)){
						//判断跳转如果是等于yes就是代理人代理 跳转代理人页面
						if("yes".equals(this.transBean.getImportMap().get("agent_persion"))){
							delayTimer.stop();
							SwingUtilities.invokeLater(new Runnable(){
								@Override
								public void run() {
									//跳转代理人授权页面
									openPanel(GlobalPanelFlag.MONEYBOX_EXIST_PROCURATOR);
								}
							});
						}else if ("no".equals(this.transBean.getImportMap().get("agent_persion"))){
							delayTimer.stop();
							SwingUtilities.invokeLater(new Runnable(){
								@Override
								public void run() {
									//跳转本人授权页面
									openPanel(GlobalPanelFlag.MONEYBOX_NEGATION_PROCURATOR);
								}
							});
						}							
					}
					...
					if(transBean.getMoney()<50000.00){
						...
						SwingUtilities.invokeLater(new Runnable(){
							...
							public void run() {
								//确认存单信息页面
								openPanel(GlobalPanelFlag.MONEYBOX_OK_INPUT_DEPINFO_PANEL);
							}
						});
					}*/
